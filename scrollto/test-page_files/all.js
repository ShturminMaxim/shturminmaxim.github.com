...
var ajaxFormSuccess = function (callback, err_callback, noscroll){
	return function(json, statusText, xhr, jqForm){	
		...
		if(json.messages == 'ok'){
			return callback(json, statusText, xhr, jqForm)
		}else{			 
			show_form_errors(json)
			if( typeof(noscroll) == 'undefined') $.scrollTo( $('.error:visible', this.form).first(), 800,	{ axis: 'y', offset: -150 } );
...
function replaceURLWithHTMLLinks(text) {
    ...
    var    text = text.replace(exp_link,'<a href="$1">$1</a>'); 
           text = text.replace(exp_user, '$1@<a href="http://www.twitter.com/$2">$2</a>');
           text = text.replace(exp_hash, '$1#<a href="http://search.twitter.com/search?q=%23$2">$2</a>');
